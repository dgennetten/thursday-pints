...
export async function fetchPhotoAvailability(): Promise<string[]> {
  const res = await fetch(`${API_BASE}/api/photos.php`)
  if (!res.ok) return []
...
export async function fetchVisitPhotos(date: string, token: string): Promise<VisitPhoto[]> {
  const res = await fetch(`${API_BASE}/api/admin/photos.php?date=${encodeURIComponent(date)}`, {
...
export async function uploadVisitPhoto(
  ...
  const res = await fetch(`${API_BASE}/api/admin/photos.php`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    body: form,
  })
